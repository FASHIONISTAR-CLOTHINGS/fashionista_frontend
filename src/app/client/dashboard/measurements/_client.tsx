...
function DeltaBadge({ delta }: { delta: ReturnType<typeof getDelta> }) {
  ...
  const color  = isUp ? "#52B788" : "#F4C430";
  const arrow  = isUp ? "↑" : "↓";
  return (
    <span
...
const ZONE_LABELS: Record<string, { label: string; accent: string }> = {
  upper: { label: "Upper Body",    accent: "#2D6A4F" },
  core:  { label: "Core",          accent: "#F4C430" },
  lower: { label: "Lower Body",    accent: "#52B788" },
  full:  { label: "Full Body",     accent: "#2D6A4F" },
};

function ZoneSection({ zone, items }: { zone: string; items: MeasurementValue[] }) {
  const cfg     = ZONE_LABELS[zone] ?? { label: zone, accent: "#2D6A4F" };
    ...
    <button
      ...
      className={`w-full text-left p-4 rounded-xl border transition-all duration-200 ${
        isSelected
          ...
          : "border-white/10 bg-white/5 hover:border-[#2D6A4F]/40 hover:bg-[#2D6A4F]/5"
      }`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            {Boolean(profile.is_default) && (
              <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-[#F4C430]/15 text-[#F4C430] border border-[#F4C430]/20 uppercase tracking-wide flex-shrink-0">
                Default
              </span>
            )}
          </div>
        ...
        <div className="text-right flex-shrink-0">
          {confidence != null && (
            <div className="text-xs font-bold" style={{ color: confidence >= 80 ? "#52B788" : confidence >= 60 ? "#F4C430" : "#DC2626" }}>
              {confidence}%
            </div>
          )}
  ...
  return (
    ...
    >
      ...
      <div className="relative w-24 h-24 mb-6">
        <div className="absolute inset-0 rounded-full border-2 border-[#2D6A4F]/20 animate-pulse" />
        <div className="absolute inset-3 rounded-full bg-[#2D6A4F]/10 flex items-center justify-center">
      ...
      <div className="grid grid-cols-2 gap-3 w-full max-w-sm mb-8 text-left">
        {[
          ...
          <div key={item.label} className="flex items-center gap-2 bg-[#2D6A4F]/10 rounded-xl px-3 py-2.5 border border-[#2D6A4F]/20">
            <span>{item.icon}</span>
      ...
      <Link
        ...
        className="inline-flex items-center gap-2 rounded-xl bg-[#F4C430] text-[#0A0A0A] font-bold text-sm px-6 py-3 hover:bg-[#C9A227] transition-colors shadow-lg shadow-[#F4C430]/20"
...
export function MeasurementsDashboard() {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#0A0A0A] via-[#0D1810] to-[#0A0A0A] px-4 py-8">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0A0A0A] via-[#0D1810] to-[#0A0A0A] px-4 py-8 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <Link
            href="/client/dashboard/measurements/scan"
            id="new-scan-btn"
            className="inline-flex items-center gap-2 rounded-xl bg-[#2D6A4F] hover:bg-[#1B4332] text-white font-semibold text-sm px-5 py-2.5 transition-colors border border-[#2D6A4F]"
          >
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="space-y-3">
              ...
              <div className="pt-2">
                <Link
                  href="/client/dashboard/measurements/scan"
                  className="w-full inline-flex items-center justify-center gap-2 rounded-xl border border-[#2D6A4F]/40 text-[#52B788] text-sm font-medium py-2.5 hover:bg-[#2D6A4F]/10 transition-colors"
                >
                  📷 Retake Body Scan
                </Link>
              </div>
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mb-6">
                      ...
                      {typeof selectedProfile.scan_confidence === "number" && (
                        <div className="text-center bg-[#2D6A4F]/10 border border-[#2D6A4F]/20 rounded-xl px-4 py-2">
                          <div className="text-xl font-bold" style={{ color: "#52B788" }}>
                    ...
                    <div className="flex items-center gap-3 mt-6 pt-6 border-t border-white/10">
                      <Link
                        href={`/client/dashboard/measurements/scan`}
                        className="flex-1 text-center text-sm font-medium rounded-xl bg-[#2D6A4F]/15 border border-[#2D6A4F]/30 text-[#52B788] py-2.5 hover:bg-[#2D6A4F]/25 transition-colors"
                      >
                        🔁 Retake Scan
                      </Link>
                      <button
                        className="flex-1 text-sm font-semibold rounded-xl bg-[#F4C430] text-[#0A0A0A] py-2.5 hover:bg-[#C9A227] transition-colors"
