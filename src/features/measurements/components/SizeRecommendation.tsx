...
export function SizeRecommendation({
  ...
  if (!result && !isPending) {
    return (
      <button
        onClick={() => mutate()}
        className="flex items-center gap-2 text-xs text-brand-gold hover:text-brand-gold/80 transition-colors group"
        id="size-recommendation-btn"
      >
        <span className="w-5 h-5 rounded-full bg-brand-gold/15 flex items-center justify-center text-[10px] group-hover:bg-brand-gold/25 transition-colors">
          ✨
        </span>
  ...
  if (isPending) {
    return (
      <div className={`flex items-center gap-2 py-2 ${className}`}>
        ...
        <span className="text-xs text-brand-gray">AI is calculating your best fit…</span>
      </div>
    );
  }
    ...
    <Card className={`p-4 ${className}`} glass>
      <div className="space-y-3">
        ...
        <div className="flex items-end gap-3">
          <div className="text-4xl font-extrabold text-brand-gold leading-none">
        ...
        {/* Fit notes */}
        {result.fit_notes && (
          <p className="text-xs text-brand-gray leading-relaxed">{result.fit_notes}</p>
        )}

        {/* Alternative sizes */}
        {result.alternative_sizes && result.alternative_sizes.length > 0 && (
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-[10px] text-brand-gray uppercase tracking-wide">Also fits:</span>
        ...
        {result.measurements_used.length > 0 && (
          <div className="pt-2 border-t border-white/8">
            <p className="text-[10px] text-brand-gray uppercase tracking-wide mb-1.5">
              Based on your
            </p>
            <div className="flex flex-wrap gap-1">
              {result.measurements_used.map((m) => (
                <span key={m} className="text-[10px] bg-white/5 border border-white/10 rounded-md px-1.5 py-0.5 text-brand-gray capitalize">
                  {m.replace(/_/g, " ")}
                </span>
              ))}
            </div>
        ...
        <button
          ...
          className="text-[10px] text-brand-gray hover:text-brand-gold transition-colors"
        >
          Recalculate ↻
        </button>
      </div>
