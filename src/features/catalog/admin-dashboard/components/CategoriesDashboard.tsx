...
export function CategoriesDashboard() {
  ...
  const handleArchive = async (id: string) => {
    if (confirm("Are you sure you want to archive this category? This operation is idempotent and will archive all associated items.")) {
      try {
  ...
  // Filter and sort categories based on search and statusFilter state
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 bg-white/40 backdrop-blur-md rounded-[24px] border border-white/20 shadow-sm">
        <div>
          ...
          <p className="font-satoshi text-sm text-[#5a5a5a] mt-1">
            Organize couture lines, premium custom apparel and e-commerce offerings
          </p>
        </div>
        ...
        <div className="bg-white p-6 rounded-[24px] shadow-md space-y-4 border border-[#fda600]/30 animate-in fade-in-50 slide-in-from-top-4 duration-300">
          <div className="flex items-center justify-between border-b border-[#f4f4f4] pb-3">
            <h4 className="font-satoshi font-bold text-xl text-black">
              {editingCategory ? "Edit Category Details" : "New Fashion Category"}
            </h4>
            <Button
              variant="ghost"
              size="icon"
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div className="space-y-1.5 md:col-span-2">
                <Label htmlFor="category-name" className="font-satoshi font-medium text-black">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="category-desc" className="font-satoshi font-medium text-black">
                Luxury Description
              </Label>
              <Textarea
                id="category-desc"
                ...
                rows={3}
                placeholder="Narrate the heritage and design direction of this category..."
      ...
      <div className="bg-white rounded-[24px] p-6 shadow-sm border border-[#e5e5e5] overflow-hidden">
        {error ? (
          <div className="p-8 text-center bg-red-50/50 rounded-2xl border border-red-100 max-w-lg mx-auto my-6 space-y-3">
            ...
            <h5 className="font-satoshi font-bold text-lg text-black">Failed to Load Categories</h5>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-[#e5e5e5] pb-3">
                  <th className="font-satoshi font-medium text-sm text-[#858585] pb-3">Category Name</th>
                  <th className="font-satoshi font-medium text-sm text-[#858585] pb-3">Unique Slug</th>
                  <th className="font-satoshi font-medium text-sm text-[#858585] pb-3">Description Summary</th>
              ...
              <tbody className="divide-y divide-[#f4f4f4]">
                {filteredCategories.length > 0 ? (
                  filteredCategories.map((category: AdminCategory) => (
                    <tr key={category.id} className="hover:bg-[#fcfcfa]/60 transition-colors group">
                      ...
                      <td className="py-4 font-satoshi text-sm text-gray-500 max-w-[280px] truncate" title={category.description}>
                      ...
                      <td className="py-4">
                        ...
                        >
                          <span className={`w-1.5 h-1.5 rounded-full ${category.active ? "bg-[#20AB2C]" : "bg-[#ECB219]"}`} />
                  ...
                  <tr>
                    <td colSpan={5} className="py-12 text-center text-sm text-gray-500 font-satoshi">
                      <div className="space-y-2 max-w-sm mx-auto">
                        ...
                        <p className="font-semibold text-black">No categories found matching criteria</p>
