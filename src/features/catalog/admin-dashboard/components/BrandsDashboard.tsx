...
export function BrandsDashboard() {
  ...
  // Filter and sort brands based on search and statusFilter state
    ...
    <div className="space-y-8 bg-inherit">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 bg-white/40 backdrop-blur-md rounded-[24px] border border-white/20 shadow-sm">
        <div>
          ...
          <p className="font-satoshi text-sm text-[#5a5a5a] mt-1">
            Manage couture design houses, ateliers, and verified vendor brand profiles
          </p>
        </div>
      ...
      <div className="bg-white p-5 rounded-[20px] shadow-sm border border-[#e5e5e5] grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
        <div className="relative col-span-1 md:col-span-2">
          ...
          <Input
            placeholder="Search brands by brand name, slug or description..."
            value={search}
        ...
        <div className="bg-white p-6 rounded-[24px] shadow-md space-y-4 border border-[#fda600]/30 animate-in fade-in-50 slide-in-from-top-4 duration-300">
          <div className="flex items-center justify-between border-b border-[#f4f4f4] pb-3">
            <h4 className="font-satoshi font-bold text-xl text-black">
              {editingBrand ? "Edit Brand Details" : "New Luxury Brand House"}
            </h4>
            <Button
              variant="ghost"
              size="icon"
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div className="space-y-1.5 md:col-span-2">
                <Label htmlFor="brand-name" className="font-satoshi font-medium text-black">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="brand-desc" className="font-satoshi font-medium text-black">
                Luxury Description
              </Label>
              <Textarea
                id="brand-desc"
                ...
                rows={3}
                placeholder="Narrate the master craftsmanship philosophy and couture legacy of this brand..."
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-[#e5e5e5] pb-3">
                  <th className="font-satoshi font-medium text-sm text-[#858585] pb-3">Brand Name</th>
                  <th className="font-satoshi font-medium text-sm text-[#858585] pb-3">Unique Slug</th>
                  <th className="font-satoshi font-medium text-sm text-[#858585] pb-3">Brand Description</th>
                ...
                {filteredBrands.length > 0 ? (
                  filteredBrands.map((brand: any) => (
                    <tr key={brand.id} className="hover:bg-[#fcfcfa]/60 transition-colors group">
                      ...
                      <td className="py-4 font-satoshi text-sm text-gray-500 max-w-[280px] truncate" title={brand.description}>
                        {brand.description || <span className="italic text-gray-300">No description provided</span>}
                      </td>
                      <td className="py-4">
                        <span
                        ...
                        >
                          <span className={`w-1.5 h-1.5 rounded-full ${brand.active ? "bg-[#20AB2C]" : "bg-[#ECB219]"}`} />
                  ...
                  <tr>
                    <td colSpan={5} className="py-12 text-center text-sm text-gray-500 font-satoshi">
                      <div className="space-y-2 max-w-sm mx-auto">
                        ...
                        <p className="font-semibold text-black">No brands found matching criteria</p>
